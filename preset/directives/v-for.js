...
module.exports = function ({ types: t }) {
  function JSXElementVisitor(path) {
    const { forAttrName } = options.attrName.for;

    path.traverse({ JSXElement: _JSXElementVisitor });

    function _JSXElementVisitor(path) {
      path.traverse({ JSXElement: _JSXElementVisitor });
      ...
      array = t.callExpression(
        ...
        typeof array === 'string' ? [objValueStr2AST(array, t)] : [array]
      );
      ...
      function getAndRemoveForBinding(openingElement) {
        ...
        if (forBinding.type === 'JSXExpressionContainer' && forBinding.expression) {
          if (forBinding.expression.type === 'StringLiteral') return forBinding.expression.value;
          if (forBinding.expression.type === 'BinaryExpression') return forBinding.expression;
        }
      }
      ...
      function parseForBinding(forBinding) {
        ...
        if (typeof forBinding === 'string') {
          ...
          if (operator !== 'in' && operator !== 'of') throw new Error(`Operator ${operator} not allowed here, using "in" instead!`);
          if (left.type === 'Identifier') params = [left.name];
          else if (left.type === 'SequenceExpression') {
            params = left.expressions.map(i => i.name);
          }

          if (right.type === 'Identifier') array = right.name;
          else array = right;
        }
        return [params, array];
      }
